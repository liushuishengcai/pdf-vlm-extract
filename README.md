...
本项目默认使用**阿里云通义千问视觉大模型（Qwen-VL）**，在 394 页中文扫描版 PDF（《龙头主力操盘术》）的完整提取中验证了可靠性；跨平台模型选型实测见下方「跨平台模型实测（2026-08-11）」章节。
...
> **注意**：替换模型后效果可能不同，模型选型参考上方「跨平台模型实测（2026-08-11）」章节。
